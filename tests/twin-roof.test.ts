...
describe("covered-porch roof render partition", () => {
  ...
  it("gives every opaque wing finish a non-overlapping plan interval", () => {
    ...
    expect(wingPorch).toEqual({
      ...
      larchSoffitEndYmm:
        HOUSE.porches.wingEnd.frontYmm - WING_PORCH_SOFFIT_FRONT_SETBACK_MM,
      portalRakeStartYmm:
  ...
  it("clips only renderer surfaces while preserving the documented overhang", () => {
    ...
    expect(renderPlan.wingPorch.larchSoffitEndYmm).toBe(22_015);
...
describe("wing porch portal head profiles", () => {
  const heads = deriveWingPorchPortalHeadProfiles();
  const clearanceMm = HOUSE.porches.wingEnd.ceilingClearanceMm;

  it("keeps every head top at least one ceiling clearance under its roof plane", () => {
    expect(heads.map(({ id }) => id)).toEqual([
      "PORCH-SUPPORT-WEST",
      "PORCH-SUPPORT-EAST",
    ]);
    for (const head of heads) {
      const faceId =
        head.id === "PORCH-SUPPORT-WEST" ? "WING_INNER" : "WING_OUTER";
      for (const point of head.profile) {
        const roofMm = roofHeightMm(faceId, point.alongMm, head.startYmm);
        expect(point.elevationMm, `${head.id}:${point.alongMm}`).toBeLessThanOrEqual(
          roofMm - clearanceMm + 1e-9,
        );
        expect(point.elevationMm).toBeGreaterThanOrEqual(
          ACTIVE_JOINED_ROOF_PARAMETERS.eavesElevationMm,
        );
      }
    }
  });

  it("clamps the eaves-side corners at the crown crossing", () => {
    const [west, east] = heads;
    const crossingRunMm =
      (clearanceMm *
        (ACTIVE_JOINED_ROOF_PARAMETERS.wingRidgeXmm -
          ACTIVE_JOINED_ROOF_PARAMETERS.wingInnerEaveXmm)) /
      (ACTIVE_JOINED_ROOF_PARAMETERS.ridgeElevationMm -
        ACTIVE_JOINED_ROOF_PARAMETERS.eavesElevationMm);

    expect(west.profile).toHaveLength(3);
    expect(west.profile[0].elevationMm).toBe(3_125);
    expect(west.profile[0].alongMm).toBeCloseTo(21_040 + crossingRunMm, 9);
    expect(west.profile[2]).toEqual({
      alongMm: 21_540,
      elevationMm: wingInnerRoofHeightMm(21_540) - clearanceMm,
    });

    expect(east.profile).toHaveLength(3);
    expect(east.profile[1].elevationMm).toBe(3_125);
    expect(east.profile[1].alongMm).toBeCloseTo(28_040 - crossingRunMm, 9);
    expect(east.profile[2]).toEqual({
      alongMm: 27_540,
      elevationMm: wingOuterRoofHeightMm(27_540) - clearanceMm,
    });
  });
});
