...
import { INTERIOR_DOORS, INTERIOR_ROOMS, KITCHEN_BEARING_WALL, KITCHEN_BEARING_WALL_EAST, KITCHEN_ISLAND, KITCHEN_RUN } from "./twin-interior";
...
const islandTop=KITCHEN_ISLAND.worktopRectMm;
const KITCHEN_NOTE = `Samostatný kuchynský ostrovček má pracovnú dosku ${mm(islandTop.x1-islandTop.x0)} × ${mm(islandTop.y1-islandTop.y0)} mm vo výške ${KITCHEN_RUN.counterHeightMm} mm. Jeho pravá hotová hrana lícuje s koncom pracovnej dosky linky oproti. Medzi ostrovčekom a plnou bočnou linkou zostáva ${mm(KITCHEN_ISLAND.sidePassageMm)} mm široký priechod. Pred zadnou linkou zostáva ${mm(KITCHEN_ISLAND.workAisleMm)} mm medzi doskami (${mm(KITCHEN_RUN.peninsulaRectMm.y0-KITCHEN_RUN.rectMm.y1)} mm medzi korpusmi). Drez 600 × 400 mm je na ostrovčeku, umývačka v krajnom východnom module smerom k malému oknu EAST-04. Od drezu ju oddeľuje zásuvková skrinka 600 mm; otvára sa do pracovnej uličky. Napravo od drezu je ${mm(KITCHEN_ISLAND.preparationWidthMm)} mm súvislej prípravnej plochy. Indukcia 800 mm je v nike pri stene, odsávač 880 mm v horných skrinkách, bez komína pod katedrálovým stropom. Chladnička a rúra so zasúvacími dvierkami tvoria dvojicu vysokých skríň; rúra je vo výške 850–1 445 mm. Všetky kuchynské čelá sú z prírodného dubu; pracovné dosky a obklad niky majú čierny saténový povrch. Zo strany obývačky má ostrovček plytké uzavreté úložisko 260 mm. Pracovnú plochu osvetľuje štíhla línia nad ostrovčekom a svetlo pod skrinkami. Hrana smerom k obývačke (${mm(PENINSULA_WORKTOP_EDGE_YMM)} mm) naďalej lícuje s ostením terasových dverí do dvora. Bočná linka pod oknom EAST-04 zostáva v pôvodnej plnej dĺžke; jej doska končí na tej istej línii smerom k obývačke ako ostrovček. Pod parapetom pokračuje doska bez obkladu.`;
...
const A: LivingLayout = {
  ...
  summary:
    "TV zostava medzi kachľami a záhradným presklením, L-sedačka pri východnej stene oproti nej a jedálenský stôl pre šesť medzi kuchynským ostrovčekom a sedačkou.",
...
const B: LivingLayout = {
  ...
  notes: [
    ...
    "L-sedačka 2 900 × 1 230 mm s ležadlom 1 100 × 2 750 mm je otočená chrbtom ku kuchyni a stojí 200 mm od východnej steny. Na požiadavku stavebníka z 13. 9. 2026 je posunutá o 200 mm ku kuchynskému drezu. Medzi operadlom (14 600 mm) a hranou pracovnej dosky ostrovčeka (13 800 mm) zostáva priechod 800 mm k jeho obslužnej strane; koberec 3 150 × 3 400 mm spája sedačku s TV stenou a dva oválne stolíky sú v rohu L.",
    "Jedálenský stôl 900 × 2 000 mm pre šesť osôb stojí pozdĺž západnej steny (22 500–23 400 × 15 400–17 400 mm) s tromi stoličkami na každej dlhšej strane: pri stene ostáva 957 mm na odsunutie stoličky, na východ 1 041 mm k sedačke (911 mm za sediacim). Terasové posuvné dvere, nástup z chodby aj pás 1 838 mm pri ostrovčeku zostávajú voľné.",
    ...
    "Kuchynská linka, ostrovček s varnou doskou aj bočné rameno pri východnej stene sú v oboch variantoch rovnaké; mení sa len obývacia a jedálenská zóna.",
    KITCHEN_NOTE,
    BEARING_WALL_NOTE,
  ],
};
