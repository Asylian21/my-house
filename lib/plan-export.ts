...
const WALL_TYPE_LIST:WallType[]=[
  ...ACOUSTIC_ASSEMBLIES.map(a=>({code:a.code,kind:'acoustic' as const,nominal:a.totalMm,label:a.name})),
...
export type WallClass='exterior'|'insulation'|'bearing'|'partition'|'board'|'acoustic'|'mineral-wool'|'plaster'|'gypsum-board';
...
export const interiorWallClass=(mesh:PlanMesh,kind:WallKind):WallClass=>{
  ...
  return acoustic?acoustic.material==='masonry'?'partition':acoustic.material:wallClass(kind,thicknessOf(mesh.rect));
};
...
export function facadeSegments(def:FacadeDef):FacadeSegment[] {
  ...
  const list=[...spans.values()].filter(s=>s.b>def.from&&s.a<def.to).sort((p,q)=>p.a-q.a||q.b-p.b);
  const out:FacadeSegment[]=[];let cursor=def.from;
  for(const s of list){
    // At outer corners the masonry layer is inset behind the perpendicular wall's insulation: it lies inside the insulation span of the same segment.
    const last=out[out.length-1];
    if(last&&last.kind!=='open'&&s.a>=last.a-1&&s.b<=last.b+1&&(s.a>last.a+1||s.b<last.b-1)){last.meshes.push(...s.meshes);continue;}
...
const exteriorType=wallType('exterior',EXTERIOR_WALL.totalMm);
/** Facade layer thickness across the wall, measured on the modelled mesh (loggia cheek runs along x). */
const layerThickness=(m:PlanMesh)=>/bočná/.test(m.name)?m.rect.x1-m.rect.x0:m.rect.y1-m.rect.y0;
export const WALL_SOLIDS:WallSolid[]=[
  ...
  ...wallItems.flatMap(item=>{const a=ACOUSTIC_WALL_SPECS.find(w=>w.mark===item.id)?.assembly;return a?[{id:item.id,rect:item.rect,kind:'acoustic' as const,thickness:a.totalMm,type:wallType('acoustic',a.totalMm),snap:false,mesh:item.meshes[0]}]:[];}),
];
...
const rangeText=(values:number[])=>{const min=Math.min(...values),max=Math.max(...values);return min===max?`${min}`:`${min}–${max}`;};
...
export const WALL_TYPES_USED=WALL_TYPE_LIST.map(type=>{
  if(type.kind==='exterior'&&type.nominal===EXTERIOR_WALL.masonryMm){
    // The rendered masonry overlaps the floors by up to 30 mm; the documented faces give 299–304.
    const modelled=FACADE_LAYERS.filter(l=>l.layer==='masonry').map(l=>Math.min(l.thickness,EXTERIOR_WALL.totalMm-EXTERIOR_WALL.insulationMm));
    return {type,range:`${EXTERIOR_WALL.masonryMm} · v modeli po líce podláh 299–304 · rendrované ${rangeText(modelled)}`};
  }
...
export interface MaterialLegendEntry { cls:WallClass; codes:string; text:string; layers?:readonly {material:'masonry'|'mineral-wool'|'plaster'|'gypsum-board';thicknessMm:number}[] }
...
export const MATERIAL_LEGEND:MaterialLegendEntry[]=(()=>{
  ...
  const out:MaterialLegendEntry[]=[
    {cls:'exterior',codes:codes('exterior'),text:`Nosné obvodové murivo z keramických tvaroviek hr. ${EXTERIOR_WALL.masonryMm} mm, na systémovú murovaciu maltu; obvodová stena celkom ${EXTERIOR_WALL.totalMm} mm. Murované piliere a voľné úseky stien na terasách a v lodžii (SO${EXTERIOR_WALL.totalMm/10}) sú plné murivo hr. ${EXTERIOR_WALL.totalMm} mm bez zateplenia`},
    {cls:'insulation',codes:codes('insulation'),text:`Kontaktný zatepľovací systém (ETICS) z minerálnej vlny hr. ${nominal('insulation')} mm na vonkajšom líci obvodového muriva, s tenkovrstvovou omietkou`},
...
export function clearRects(room:PlanRoom):RectMm[] {
  const walls=WALL_SOLIDS.filter(s=>(s.kind==='exterior'||s.kind==='insulation')&&!s.snap);
  return room.rectsMm.map(rect=>{
    let r={...rect};
    // Two passes: the insulation layer sits outside the masonry, so a layered wall trims the floor in two steps.
    for(const side of ['y1','y0','x1','x0','y1','y0','x1','x0'] as const){
