...
export const PLAN_ROOM_NOTES:Record<string,string[]>={
  ...
  'ROOM-1-06':['Priečka sa posunula o 230 mm. Medzi priečkami je 1 529 mm; plocha pôdorysu vzrástla z 2,34 na 2,75 m². Po 10 mm obklade má šírku približne 1 509 mm. WC je vycentrované, umývadlo má 550 × 350 mm a puzdrové dvere nezaberajú priestor.'],
  ...
  'ROOM-1-07':['Zostava podľa dohody zo 16.–17. 2. 2026: DEFRO Firewood Duo Plus 19 kW, násypka 180 kg a DBO-S 1 000 l. Pre 19 kW uvádza návod 1 000 l; objem sa neznižuje na 800 l.','Kotol: obal 1 238 × 1 298 × 1 391 mm na 50 mm podstavci. Odstupy po hotové obklady: 509 / 500 mm po bokoch a najmenej 540 mm vzadu. Predný pás 2 000 mm sa meria od čela telesa; zahŕňa aj horák.','Nádrž: Ø1 106 × 1 913 mm s izoláciou, Ø897 mm bez nej. Dvojkrídlové dvere s otvorom 1 700 mm majú konzervatívny čistý pás 1 352 mm a prah 20 mm. Nádrž prevážať zvislo na 100 mm podvozku podľa vyznačenej trasy, pred montážou prípojok.','Uzavretá rezerva pri západnej stene, mimo pásu obsluhy kotla, má 500 × 450 × 1 200 mm a obsahuje tri modelové 15 kg vrecia a kompaktný vysávač. Jej požiarne oddelenie zatiaľ nie je schválené. Návod požaduje odstup od horľavých predmetov; voľné vrecia sem nepatria.','Priechod ku skrini bol v modeli overený pre šírku 550 mm; je tesný a závisí od potvrdenia presného tvaru a osadenia násypky. Nad skriňou je rezerva na hydrauliku 500 × 450 × 1 100 mm. Presný rozmer DEFROmat, expanzia, potrubia, vetranie, komín a požiarna skladba vyžadujú dokončenie montážneho projektu. Model overuje priestor, nie povolenie na inštaláciu.'],
};
...
function semantic(mesh:PlanMesh):[string,string,PlanCategory] {
  ...
  if(tag==='VACUUM')label='Kompaktný vysávač · rezerva';
  if(tag==='STORAGE-CABINET')label='Uzavretá skladová rezerva';
...
export const PLAN_ITEMS:PlanItem[]=[...grouped.values()].map(item=>{
  ...
  if(item.id.startsWith(heating.storage.id)){
    ...
    if(item.id.endsWith('-STORAGE-CABINET'))item.note='Rozmerová rezerva s posuvným čelom. Požiarne oddelenie musí schváliť projektant; nejde o vybraný certifikovaný výrobok.';
