...
import { HOUSE } from '@/versions/v2/lib/twin-site';
import { area, rect } from '@/versions/v2/lib/floor-plan-concept';
import { createExperiment, DEFAULT_EXPERIMENT, normalizeExperiment, bedroomDoorOffsetMax, type ExperimentSettings } from '@/versions/v2/lib/floor-plan-experiment';
import './studio.css';
...
export function ExperimentalFloorPlanStudio({rawSettings,setSettings,variant,onVariantChange,view}:ExperimentStudioProps) {
  ...
  return <main className="fp-studio fp-experiment">
    <header className="fp-header"><Link href="/v2" className="fp-back" aria-label="Späť na 3D dom"><ArrowLeft size={18}/></Link><div className="fp-brand">DOM<span>/</span><h1>Dispozičné štúdio</h1><span className="fp-badge">VARIANT E</span></div><button className="fp-export" onClick={exportPlan}><Download size={16}/><span>Stiahnuť plán E</span></button></header>
...
<svg ref={svg} className="fp-plan" viewBox={full?'5500 -23100 23700 21500':'5550 -12350 17300 10850'} role="img" aria-label="Koncept E so spálňou do dvora, súkromným šatníkom a zalomeným vstupom">
      ...
      <div className="fp-source"><strong>Teoretický 2D koncept</strong><p>Rozmery a priechody sú modelované v milimetroch. Nejde o realizačný výkres ani potvrdenie statiky. Nábytok mimo rodičovskej časti je schematický.</p><Link href="/v2">Otvoriť existujúci 3D dom<ArrowUpRight size={14}/></Link></div>
    </aside></div>
  </main>;
}
