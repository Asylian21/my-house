...
export class ArchvizPresentation {
  ...
  private async load() {
    ...
    try {
      const manifestResponse = await fetch("/v2-assets/assets/archviz/export-manifest.json", { signal: this.abort.signal });
      ...
      for (const file of manifest.files) {
        ...
        const response = await fetch(`/v2-assets/assets/archviz/${file.file}`, { signal: this.abort.signal });
      ...
      if (this.grassContainer) {
        const response = await fetch("/v2-assets/assets/archviz/grass-placements.json", { signal: this.abort.signal });
