...
export function DocumentationStudio({initialManual=false}:{initialManual?:boolean}) {
  ...
  const results=useMemo(()=>searchPlanItems(query,roomId,category),[query,roomId,category]);
  ...
  const exportCsv=()=>{
    const rows=[['Miestnosť','Prvok','Súčasť','ID','X min (mm)','Y min (mm)','Rozmer X (mm)','Rozmer Y (mm)','Výška (mm)','Spodná hrana (mm)','Horná hrana (mm)'],...PLAN_ITEMS.flatMap(item=>item.meshes.map(mesh=>[PLAN_ROOMS.find(r=>r.id===item.roomId)?.name??'Terasy pri dome',item.name,mesh.name,mesh.id,mesh.rect.x0,mesh.rect.y0,mesh.rect.x1-mesh.rect.x0,mesh.rect.y1-mesh.rect.y0,mesh.z1-mesh.z0,mesh.z0,mesh.z1]))];
    const csv=rows.map(row=>row.map(v=>`"${String(v).replaceAll('"','""')}"`).join(';')).join('\r\n');download('dom-variant-c-kompletny-supis.csv','\uFEFF'+csv,'text/csv;charset=utf-8');setAnnouncement('Kompletný súpis bol stiahnutý.');
  };
  const exportSvg=()=>{
  ...
  return <main className="pd-root" data-mobile-panel={mobilePanel} data-manual={manualOpen}>
    ...
    <div className="pd-workspace">
      <aside className="pd-library" aria-label="Miestnosti a súpis predmetov"><div className="pd-search"><Search size={17}/><input aria-label="Hľadať prvky v zvolenej oblasti" value={query} placeholder="Nájsť nábytok, stenu…" onChange={event=>{setQuery(event.target.value);setLeftTab('objects');}}/>{query&&<button aria-label="Vymazať hľadanie" onClick={()=>setQuery('')}><X size={14}/></button>}</div>
        <div className="pd-library-tabs"><button aria-pressed={leftTab==='rooms'} onClick={()=>setLeftTab('rooms')}>Miestnosti <span>{PLAN_ROOMS.length}</span></button><button aria-pressed={leftTab==='objects'} onClick={()=>setLeftTab('objects')}>Prvky <span>{PLAN_ITEMS.length}</span></button></div>
      ...
      <section className="pd-drawing" aria-label="Interaktívny pôdorys">
        ...
        <div className={`pd-canvas pd-tool-${tool}`} ref={canvas}>
          <svg ref={svg} className="pd-plan" viewBox={`${view.x} ${view.y} ${view.width} ${view.height}`} tabIndex={0} aria-label="Pôdorys. Ťahaním posúvajte, kolieskom približujte. Prvok vyberiete kliknutím alebo zo zoznamu." onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp} onClick={onPlanClick} onDoubleClick={()=>{if(tool!=='measure')fit(component?.rect??selected?.rect??room?.bounds??PLAN_FULL_BOUNDS);}} onKeyDown={event=>{
            ...
            <PlanGeometry layers={layers} details={details||zoom>250} overhead={overhead} selectedId={selectedId} componentId={componentId} roomId={roomId} labels={zoom<600&&!(roomId==='ROOM-1-07'&&clearances)} dimensions={dimensions} unit={unit} unitsPerPixel={view.width/canvasWidth}/>
      ...
      <aside className="pd-inspector" aria-label="Detail a rozmery vybraného prvku">
        {selected?<><div className="pd-inspector-heading"><span className="pd-overline">{PLAN_CATEGORIES[selected.category]}</span><button aria-label="Zavrieť detail" onClick={()=>{setSelectedId(null);setComponentId(null);setMobilePanel('plan');requestAnimationFrame(()=>svg.current?.focus({preventScroll:true}));}}><X size={18}/></button></div><h2 ref={inspectorTitle} tabIndex={-1}>{selected.name}</h2><p className="pd-item-room">{selectedRoom?`${selectedRoom.number} · ${selectedRoom.name}`:'Terasy pri dome'}</p>
          ...
          <div className="pd-inspector-bottom"><span>{numberSk(PLAN_ITEMS.length)} prvkov</span><span>{numberSk(allParts)} súčastí</span></div>
        </>}
      </aside>
    </div>
    <ExportPlan/>
...
const ExportPlan=memo(function ExportPlan(){return <div id="pd-export-source" hidden><StaticPlan viewBox={fitPlanRect(PLAN_FULL_BOUNDS,1.2,0)}/></div>;});

function RoomGuidance({roomId}:{roomId:string}){
  const notes=PLAN_ROOM_NOTES[roomId];
...
const DocumentationManual=memo(function DocumentationManual({printParts,setPrintParts,setManualOpen}:{printParts:boolean;setPrintParts:(value:boolean)=>void;setManualOpen:(value:boolean)=>void}) {
  return <div className="pd-print" aria-label="Manuál domu"><div className="pd-manual-toolbar"><button onClick={()=>setManualOpen(false)}><ArrowLeft size={17}/>Späť na pôdorys</button><label><input type="checkbox" checked={printParts} onChange={e=>setPrintParts(e.target.checked)}/>Priložiť všetkých {numberSk(allParts)} súčastí</label><button className="pd-primary" onClick={()=>window.print()}><Printer size={17}/>Tlačiť / uložiť PDF</button></div>
      <section className="pd-paper pd-cover"><span className="pd-overline">DOM / 01 / PRÍZEMIE</span><h2>Manuál vášho domu.</h2><p>Variant C · pôdorys a súpis aktuálneho 3D modelu</p><div id="pd-print-overview"><StaticPlan viewBox={fitPlanRect(PLAN_FULL_BOUNDS,1.2,0)}/></div><div className="pd-cover-metrics"><span>{PLAN_ROOMS.length} miestností</span><span>{PLAN_ITEMS.length} prvkov</span><span>{numberSk(allParts)} súčastí</span></div><h3>Ako čítať tento manuál</h3><p>Všetky tabuľkové rozmery sú v milimetroch. X (↔) a Y (↕) sú rozmery v pôdoryse, výška smeruje nahor. Rozsah modelu zahŕňa presahy a všetky súčasti. Pri oblých a natočených prvkoch uvádzame obdĺžnik, ktorý ich obopína. Kóty majú prednosť pred meraním na vytlačenej stránke.</p><p>Dokumentácia pokrýva prízemie: interiér, obvodové steny, výplne a kryté priestory a vonkajšie sedenie pri dome. Podklady zachytávajú model, nie zameranie stavby ani výrobný výkres. Záhrada, podzemné rozvody a strešný plášť nie sú obsahom týchto listov.</p><p>Pri voľnom meraní v aplikácii záleží na presnosti vybraných bodov. Presné rozmery jednotlivých prvkov nájdeš v ich detaile a súpise.</p></section>
      {PLAN_ROOMS.map(r=><section className="pd-paper pd-room-sheet" key={r.id}><div className="pd-paper-heading"><div><span className="pd-overline">LIST {r.number}</span><h2>{r.name}</h2></div><strong>{numberSk(r.area,2)} m²</strong></div><div className="pd-room-sheet-plan"><StaticPlan viewBox={fitPlanRect(r.bounds,1.6,800)} roomId={r.id}/></div><div className="pd-room-specs"><span>Svetlá výška: {formatMm(r.clearHeightMm)}{r.ceiling==='VAULTED_TO_RIDGE'?' pri stene · šikmý strop do 4 850 mm':''}</span><span>Podlaha: {r.floor==='VINYL'?'vinyl':r.floor==='TILE'?'dlažba':'epoxid'}</span></div><RoomGuidance roomId={r.id}/>{r.id==='ROOM-1-07'&&<TechnicalLegend/>}<p>Časti čistej podlahy: {r.rectsMm.map(p=>`${numberSk(p.x1-p.x0)} × ${numberSk(p.y1-p.y0)}`).join(' + ')} mm. Plocha je súčtom týchto častí.</p><table><thead><tr><th>Prvok</th><th>X ↔</th><th>Y ↕</th><th>Výška</th><th>Diely</th></tr></thead><tbody>{PLAN_ITEMS.filter(item=>item.roomId===r.id).map(item=><tr key={item.id}><td>{item.name}{item.product&&<small>{item.product.dimensions} · katalóg</small>}{item.nominal&&<small>Rozmer podľa zadania modelu</small>}{item.opening&&<small>Rozmery otvoru: {numberSk(item.opening.width)} × {numberSk(item.opening.height)} mm</small>}</td><td>{numberSk((item.nominal??item.rect).x1-(item.nominal??item.rect).x0)}</td><td>{numberSk((item.nominal??item.rect).y1-(item.nominal??item.rect).y0)}</td><td>{numberSk(item.z1-item.z0)}</td><td>{item.meshes.length}</td></tr>)}</tbody></table><footer>DOM · C · Rozmery v mm · Rozsah celku vrátane presahov, ak nie je uvedené inak.</footer></section>)}
      <section className="pd-paper"><span className="pd-overline">VONKAJŠIE SEDENIE</span><h2>Terasy pri dome</h2><p>Nábytok z aktuálneho 3D modelu. Rozmery v mm.</p><table><thead><tr><th>Prvok</th><th>X ↔</th><th>Y ↕</th><th>Výška</th></tr></thead><tbody>{PLAN_ITEMS.filter(item=>item.roomId==='EXTERIOR').map(item=><tr key={item.id}><td>{item.name}</td><td>{numberSk(item.rect.x1-item.rect.x0)}</td><td>{numberSk(item.rect.y1-item.rect.y0)}</td><td>{numberSk(item.z1-item.z0)}</td></tr>)}</tbody></table></section>
      {printParts&&<section className="pd-paper pd-appendix"><h2>Kompletný rozpis súčastí</h2><p>Každá viditeľná fyzická súčasť exportovaných zostáv. Rozmery sú v mm.</p>{PLAN_ITEMS.map(item=><div key={item.id}><h3>{PLAN_ROOMS.find(r=>r.id===item.roomId)?.number} · {item.name}</h3><table><thead><tr><th>Súčasť</th><th>X ↔</th><th>Y ↕</th><th>Výška</th><th>Od podlahy</th></tr></thead><tbody>{item.meshes.map(mesh=><tr key={mesh.id}><td>{mesh.name.split(' · ').slice(-1)[0]}</td><td>{numberSk(mesh.rect.x1-mesh.rect.x0)}</td><td>{numberSk(mesh.rect.y1-mesh.rect.y0)}</td><td>{numberSk(mesh.z1-mesh.z0)}</td><td>{numberSk(mesh.z0)}</td></tr>)}</tbody></table></div>)}</section>}
    </div>;
});
