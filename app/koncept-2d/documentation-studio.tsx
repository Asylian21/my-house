    ...
    <div className="pd-workspace">
      ...
      <aside id="pd-inspector" className="pd-inspector" aria-label="Výber a informácie o návrhu">
        ...
        <div id="pd-inspector-panel-design" className="pd-inspector-content pd-design-content" role="tabpanel" aria-labelledby="pd-inspector-tab-design" tabIndex={0} hidden={inspectorTab!=='design'}>
          ...
          <details className="pd-design-section"><summary><span>Akustické steny<small>SA30 · 300 mm / H200 · 200 mm</small></span><ChevronDown size={17}/></summary><section className="pd-acoustic-index"><p>AK-01 a AK-02: dvojplášťová skladba 300 mm. AK-03: LeierPLAN 10 a predstena Silentboard, spolu 200 mm vrátane omietok.</p>{ACOUSTIC_WALL_SPECS.map(w=><button key={w.mark} onClick={()=>{const item=PLAN_ITEM_BY_ID.get(w.mark);if(item)choose(item,null,true);}}><strong>{w.mark}</strong><span>{w.location}</span><ChevronRight size={14}/></button>)}</section></details>
...
const DocumentationManual=memo(function DocumentationManual({archive,printParts,setPrintParts,livingLayout,heatingLayout}:{archive:boolean;printParts:boolean;setPrintParts:(value:boolean)=>void;livingLayout:LivingLayoutId;heatingLayout:HeatingLayoutId}) {
  ...
  return <div className="pd-print" aria-label="Manuál domu"><div className="pd-manual-toolbar"><Link href={designHref('/podorys',{livingLayout,heatingLayout},archive)}><ArrowLeft size={17}/>Späť na pôdorys</Link><label><input type="checkbox" checked={printParts} onChange={e=>setPrintParts(e.target.checked)}/>Priložiť všetkých {numberSk(allParts)} súčastí</label><button className="pd-primary" onClick={()=>window.print()}><Printer size={17}/>Tlačiť / uložiť PDF</button></div>
      ...
      <section className="pd-paper pd-acoustic-sheet"><div className="pd-paper-heading"><div><span className="pd-overline">DETAIL SA30 · 13. 9. 2026</span><h2>Akustické steny</h2></div><strong>300 mm</strong></div><div className="pd-acoustic-sheet-grid"><div><StaticPlan viewBox={{x:12800,y:-11200,width:4200,height:8500}} labels={false} livingLayout={livingLayout} heatingLayout={heatingLayout}/></div><AcousticWallDetail/></div><table><thead><tr><th>Označenie</th><th>Umiestnenie</th><th>Skladba</th></tr></thead><tbody>{ACOUSTIC_WALL_SPECS.filter(w=>w.assembly.code==='SA30').map(w=><tr key={w.mark}><td>{w.mark}</td><td>{w.location}</td><td>SA30 · 100 / 100 / 100 mm</td></tr>)}</tbody></table><p>{ACOUSTIC_ASSEMBLY.structuralNote}</p><p>{ACOUSTIC_ASSEMBLY.acousticNote}</p><p>{ACOUSTIC_ASSEMBLY.bathroomNote}</p><footer>DOM · C/B/B · Rozmery v mm · Líca a dispozícia zachované.</footer></section>
      <section className="pd-paper pd-acoustic-sheet pd-office-acoustic-sheet"><div className="pd-paper-heading"><div><span className="pd-overline">DETAIL AK-03 · H200</span><h2>Pracovňa a sprcha</h2></div><strong>200 mm · Rw ≈ 58 dB*</strong></div><div className="pd-acoustic-sheet-grid"><div><StaticPlan viewBox={{x:21100,y:-9100,width:7000,height:6000}} labels={false} livingLayout={livingLayout} heatingLayout={heatingLayout}/><p>Stavebný otvor kúpeľne 800 mm a krídlo 700 mm sú súmerné podľa pozdĺžnej osi X chodby. Os Y = 7 101,5 mm; chodba má 1 099 mm.</p><table><thead><tr><th>Kontrolná kóta</th><th>mm</th></tr></thead><tbody><tr><td>Južné ostenie otvoru kúpeľne · Y</td><td>6 701,5</td></tr><tr><td>Severné ostenie otvoru kúpeľne · Y</td><td>7 501,5</td></tr><tr><td>Ostenie pri pracovni / pri kúpeľni</td><td>80 / 99,5</td></tr><tr><td>Posun dverí a vstupného rohu pracovne k ulici</td><td>30</td></tr></tbody></table></div><AcousticWallDetail mark="AK-03"/></div><p>{OFFICE_ACOUSTIC_ASSEMBLY.acousticNote}</p><p>{OFFICE_ACOUSTIC_ASSEMBLY.structuralNote}</p><p>{OFFICE_ACOUSTIC_ASSEMBLY.bathroomNote}</p><footer>DOM · C/B/B · Rozmery v mm · 200 mm vrátane omietok a dosiek, bez finálnych povrchov · * Predbežný výpočet Rw, presnú zostavu potvrdiť.</footer></section>
