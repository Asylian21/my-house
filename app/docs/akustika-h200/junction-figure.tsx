...
export function H200JunctionFigure() {
  ...
  const wallEnd = 555;
  const layers = [
    { id: 'bath-plaster', from: 6587, to: 6602, fill: '#e1e7e8' },
    { id: 'brick', from: 6487, to: 6587, fill: `url(#${id}-brick)` },
    { id: 'cavity-plaster', from: 6472, to: 6487, fill: '#e1e7e8' },
    { id: 'wool', from: 6427, to: 6472, fill: `url(#${id}-wool)` },
    { id: 'inner-board', from: 6414.5, to: 6427, fill: '#7897a8' },
    { id: 'outer-board', from: 6402, to: 6414.5, fill: '#436d85' },
  ];
  ...
  return <figure className="h200-junction-figure">
    ...
    <svg viewBox="0 0 1000 820" role="img" aria-labelledby={`${id}-title ${id}-description`} style={{ width: '100%', height: 'auto', display: 'block' }}>
      ...
      <desc id={`${id}-description`}>Pôdorysná schéma zachováva pevný spoločný pilier a polohu oboch dverí. Oddelenie má dve spojené časti: päť milimetrov pri koncoch oboch dosiek a päť milimetrov nad výplňou pracovňového ostenia. Z nominálnych osemdesiatich milimetrov zostáva sedemdesiatpäť milimetrov pevnej výplne a päť milimetrov pripojovacej škáry. Zväčšenie ukazuje obe škáry po celej výške steny.</desc>
      <defs>
      ...
      <g fontFamily="Arial, sans-serif" fill={ink}>
        ...
        <text x="32" y="68" fontSize="17" fill={muted}>Pôdorysná schéma · polohy dverí a hrúbka H200 zostávajú</text>
        ...
        <path d={`M140 ${y(6701.5)}H${pierRight}V${y(6397)}H${nibRight}V${y(6322)}H140V${y(6412)}H${x(22540)}V${y(6552)}H140Z`} fill={pier} stroke={ink} strokeWidth="1.4" />
        {layers.map(layer => <rect key={layer.id} x={layer.id.includes('board') ? boardStart : pierRight} y={y(layer.to)} width={wallEnd - (layer.id.includes('board') ? boardStart : pierRight)} height={(layer.to - layer.from) * .8} fill={layer.fill} stroke="#53636b" strokeWidth="1" />)}

        {/* J1: X22783–22788/Y6402–6427; J2: X22783–22842/Y6397–6402. */}
        <rect x={pierRight} y={y(6427)} width="4" height="20" fill={joint} />
        <rect x={pierRight} y={y(6402)} width={nibRight - pierRight} height="4" fill={joint} />
        <path d={`M${pierRight + 2} ${y(6427) + 1}V${y(6402) + 2}H${nibRight - 1}`} fill="none" stroke={joint} strokeWidth="3" />
        <rect x={pierRight - 9} y={y(6427) - 8} width={nibRight - pierRight + 20} height="41" rx="5" fill="none" stroke={joint} strokeWidth="1.4" strokeDasharray="5 4" />
        ...
        {dim(326, y(6701.5), y(6602), '99,5 mm')}
        {dim(326, y(6402), y(6322), '80 mm')}
        <text x="369" y="498" fontSize="17" fill={joint}>75 + 5 mm</text>
        <text x="370" y="326" fontSize="19" fontWeight="700">H200 · 200 mm</text>
        <text x="370" y="350" fontSize="16">obe dosky spolu</text>
        ...
        <rect x="742" y="336" width="218" height="28" fill="#7897a8" stroke="#53636b" />
        <rect x="742" y="364" width="218" height="28" fill="#436d85" stroke="#53636b" />
        ...
        <text x="832" y="353" textAnchor="middle" fontSize="16" fill="#fff">vnútorná doska</text>
        <text x="832" y="383" textAnchor="middle" fontSize="16" fill="#fff">vonkajšia doska</text>
        ...
        <text x="79" y="651" fontSize="18" fontWeight="700">Konce oboch dosiek: škára 5 mm</text>
